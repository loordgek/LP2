                            ...
                            public Triple<NetworkNode, NetworkNode, ArrayDeque<Tuple<UUID, EnumFacing>>> call()
                                ...
                                while (!open.isEmpty()) {
                                    ...
                                    if (current.equals(target)) {
                                        ...
                                        ArrayDeque<Tuple<UUID, EnumFacing>> route = new ArrayDeque<>();
                                        while (help.parent != null) {
                                            route.push(new Tuple<>(help.parent.getKey().getId(), help.parent.getVal()));
                            ...
                            private void tryLookahead(NetworkNode current, EnumFacing direction) throws InterruptedException {
                                ...
                                if (node != null) {
                                    if (node.connections() != 2 || current.isDestination()) {
                                        node.h_Cost(target);
                                        node.parent = new Tuple<>(current, direction);
